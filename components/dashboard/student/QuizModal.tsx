...
export const QuizModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 sm:p-4">
      <div className="relative w-full max-w-2xl rounded-lg border border-slate-200 bg-white shadow-xl max-h-[95vh] sm:max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 p-3 sm:p-4 lg:p-6 flex-shrink-0">
          <div className="min-w-0 flex-1">
            <h2 className="text-base sm:text-lg lg:text-xl font-bold text-slate-900">Quiz</h2>
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 sm:p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-900 flex-shrink-0 ml-2"
            aria-label="Close"
          >
        ...
        {/* Progress Bar */}
        <div className="border-b border-slate-200 px-3 sm:px-4 lg:px-6 py-2 flex-shrink-0">
          ...
          {!submitted ? (
            <>
              <div className="mb-4 flex items-start gap-2">
                ...
                <p className="text-sm sm:text-base font-semibold text-slate-900">{question.question}</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {showAnswer && (
                          <div>
                            {showCorrect && <CheckCircle2 size={18} className="text-emerald-600" />}
        ...
        {/* Footer Navigation */}
        <div className="border-t border-slate-200 p-3 sm:p-4 lg:p-6 flex-shrink-0">
          {!submitted ? (
            <div className="flex items-center justify-between gap-3">
              <button
                onClick={handlePrevious}
                disabled={currentQuestion === 0}
                className="flex items-center gap-1 sm:gap-2 rounded-lg border border-slate-300 bg-white px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
                <ChevronLeft size={14} className="sm:w-4 sm:h-4" />
                <span className="hidden sm:inline">Previous</span>
              </button>

              <div className="flex-1 text-center text-[10px] sm:text-xs text-slate-500 px-2">
              ...
              {currentQuestion === questions.length - 1 ? (
                <button
                  ...
                  className="flex items-center gap-1 sm:gap-2 rounded-lg bg-[#111827] px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-white transition-colors hover:bg-[#1f2937] disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                >
                  {submitting ? (
                    <>
                ...
                <button
                  onClick={handleNext}
                  className="flex items-center gap-1 sm:gap-2 rounded-lg bg-[#111827] px-2 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-white transition-colors hover:bg-[#1f2937] flex-shrink-0"
                >
            ...
            <button
              onClick={onClose}
              className="w-full rounded-lg bg-[#111827] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#1f2937]"
            >
              Close
            </button>
          )}
